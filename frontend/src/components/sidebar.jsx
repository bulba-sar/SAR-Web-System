...
export default function Sidebar({ activeNav, setActiveNav }) {
  ...
  const NavItem = ({ id, label, icon }) => {
    ...
    return (
      <button
        ...
        className={`w-full flex flex-col items-center justify-center py-5 border-l-4 transition-all ${
          isActive 
      ...
      >
        {icon}
        <span className="text-[10px] mt-1.5 font-bold tracking-wider uppercase">{label}</span>
      </button>
    );
  };
  ...
  return (
    ...
    <div className="w-24 h-screen bg-[#f0f2f2] border-r border-zinc-200 flex flex-col justify-between py-6 z-20 shadow-sm">
      ...
      <div className="flex flex-col w-full items-center space-y-4">
        ...
        <div className="mb-4">
          <div className="w-10 h-10 bg-[#305d3d]/10 rounded-xl flex items-center justify-center border border-[#305d3d]/20">
             <span className="text-[#305d3d] font-black text-lg">B</span>
          </div>
        </div>
        ...
        <NavItem 
          id="analysis" 
          label="Analysis" 
          icon={<svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M18 20V10H14V20H18ZM12 20V4H8V20H12ZM6 20V14H2V20H6Z" /></svg>}
        />
        <NavItem 
          id="feedback" 
          label="Feedback" 
